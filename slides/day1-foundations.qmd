...
::: aside
Tomorrow (Day 2): MCPs, subagents, custom commands, dashboards, Whisper for paper discussions, and a full academic workflow.
:::
...
## Claude Code — the research workhorse
...
```
┌─────────────────────────────────────────────┐
│  1. GATHER CONTEXT   read files, search,    │
│                      run `ls`, `git log`    │
│  2. TAKE ACTION      edit file, run script  │
...
## Setup check — do this now {.your-turn}
...
## Prompting: be specific, be verifiable
...
## Context: the one resource you manage
...
```
Complex task  →  Plan Mode  →  read the plan critically
              →  approve (or redirect)  →  auto-accept execution
              →  git diff  →  commit
```

## Plan Mode: a research example
...
```
Plan Mode          Default            Auto-accept         --dangerously-
(read-only)        (ask each time)    (edits flow)        skip-permissions
   ▲                                                        ▲
   │ start here for anything new                            │ containers/CI
   └────────────── trust gradient ──────────────────────────┘
```
...
::: aside
Slides built with Quarto + reveal.js — using Claude Code, of course.
:::
