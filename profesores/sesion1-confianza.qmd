...
| 8:05 – 8:15 | ☕ | Pausa 2 |
| 8:15 – 8:55 | 6 | ⭐ Literatura: `/deep-research` y la trampa de la cita |
...
## Su turno: ¿qué me falta instalar? {.your-turn}
...
![](../figures/screenshots/vscode-md-raw.png){.nostretch width=64% fig-align="center"}

Así se ve **por dentro**: los `#`, `**` y `|` a la vista. [▸ Clic en el **icono de la lupa** (arriba a la derecha) y VS Code lo lee por ustedes.]{.warm}

## …y así lo muestra VS Code {.smaller}

![](../figures/screenshots/vscode-md-preview.png){.nostretch width=74% fig-align="center"}
...
# Bloque 6 — La literatura: buscar y verificar
...
![](../figures/es/agentes-paralelo.svg){.nostretch width=86% fig-align="center"}

## No es una búsqueda: es un equipo {.smaller}

![](../figures/es/deep-research-caso.svg){.nostretch width=96% fig-align="center"}

## ¿Cuántos agentes son, exactamente? {.smaller}
...
[En una corrida real de este taller contamos **cerca de 100 agentes en total**: 6 al inicio, 26 leyendo, 75 verificando. Si pasa de 25, Claude Code avisa que es una **corrida grande**.]{.warm}
...
## ⭐ La trampa de la cita {.your-turn .smaller}

Mientras el informe se cocina, veamos el otro lado. Abran `bibliografia-borrador.md`
del proyecto de ejemplo: es lo que cualquiera de nosotros arma un domingo copiando
de Google Scholar.
...
## De vuelta al informe {.your-turn .smaller}

Ya debe haber terminado. Ábranlo y hagan con él lo mismo que acaban de aprender:

```text
De la bibliografía que armaste, verifica cada referencia contra
Crossref y dime cuáles tienen algún dato que no coincide.
```

. . .

**Qué observar:** el informe trae la fuente de cada afirmación — y aun así, la verificación encuentra cosas. **Esa doble pasada es el método.**
...
![](../figures/es/biblioteca-flujo.svg){.nostretch width=88% fig-align="center"}

## Cómo se loguean, en 40 segundos {.your-turn .smaller}

1. En el navegador, abran esta dirección **tal cual** (es JSTOR a través del proxy):

```text
https://pucp.elogim.com/auth-meta/login.php?url=https://www.jstor.org
```

2. Les pedirá **usuario y contraseña del campus virtual** — los mismos de PAIDEIA/Intranet.
3. Listo. **Dejen esa pestaña abierta**: el navegador ya quedó reconocido como PUCP.
...
## Y ahora sí: que él descargue {.your-turn .smaller}

Con la sesión abierta en el navegador, vuelvan a Claude:

```text
Estoy logueado en la biblioteca PUCP en el navegador. Toma el archivo
pendientes.md y, una por una, intenta descargar esas fuentes a papers/
usando el navegador y el proxy https://pucp.elogim.com/auth-meta/login.php?url=
Actualiza pendientes.md marcando cuáles conseguiste; para las que no,
dime dónde más buscarlas.
```
...
1. **Tiene que usar el navegador, no comandos.** La sesión vive en una cookie del navegador; si intenta bajarlo "por código" recibe la página de login. Si ven que falla así, díganle: *"usa el navegador, no curl"*.
2. **Que la página cargue no significa que haya suscripción.** Hay que llegar hasta el PDF. (Comprobado que **no** tenemos: Management Science/INFORMS ni Oxford — o sea, ni QJE ni ReStud por esta vía.)
