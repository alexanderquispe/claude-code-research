...
`/deep-research` **tarda 15–20 minutos** — así que lo echamos a andar *ahora* y seguimos la clase mientras trabaja:
...
Prioriza estudios cuantitativos e informes oficiales.
```

. . .

Y el pedido que lo convierte en material suyo — **añádanlo al mismo mensaje**:

```text
Descarga los PDFs que cites en una carpeta papers/, agrupados por tema.
```

[Corre **en segundo plano**: la sesión les queda libre. Vuelvan a mirarlo con `/workflows`.]{.warm}
...
## La solución: la biblioteca PUCP {.smaller}

El proxy de la Universidad se llama **elogim**. Ustedes se loguean **una vez** en el navegador y el agente trabaja sobre esa sesión abierta:
...
```text
Estoy logueado en la biblioteca PUCP en el navegador. De la lista de
referencias, descarga a papers/ las que estén en revistas suscritas.
Si alguna no tiene acceso, dime dónde más se puede conseguir.
```

. . .

**Tres cosas que cuestan descubrir solas:**

1. **Su cuenta personal ≠ el acceso institucional.** Con usuario propio en JSTOR solo *leen en pantalla*; el botón de descarga aparece bajo la sesión de la Universidad.
2. **La sesión va por cookie** → tiene que ser un navegador logueado. Si el agente intenta bajarlo "por código", falla: díganle *"usa el navegador"*.
3. **Que la página cargue no significa que haya suscripción.** (Comprobado que **no** tenemos: Management Science/INFORMS ni Oxford — o sea, ni QJE ni ReStud por esta vía.)
